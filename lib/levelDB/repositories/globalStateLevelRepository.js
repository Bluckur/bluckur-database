...
class GlobalStateLevelRepository {
  ...
  getGlobalStateAsync() {
    return new Promise((resolve, reject) => {
      ...
      this.globalStateLevel.createValueStream()
        ...
        .on('end', () => {
          log(`${states.length} states fetched`);
          resolve(states);
        });
    });
  }
  ...
  putStatesAsync(states) {
    return new Promise((resolve, reject) => {
      ...
      log(`putting states ... [publicKeys=${states.map((s) => { return s.publicKey; })}]`);
      this.globalStateLevel.batch(ops).then(() => {
        log(`${states.length} states put`);
        resolve();
      }).catch((err) => {
        error(err.message);
        reject(err);
  ...
  updateGlobalStateAsync(transactions) {
    return new Promise((resolve, reject) => {
      ...
      log(`updating global state ... [publicKeys:${pendingStates.map((s) => { return s.publicKey; })}]`);
      const promises = [];
      ...
      Promise.all(promises).then((states) => { return this.putStatesAsync(states); }).then(() => {
        log(`${pendingStates.length} states updated`);
        resolve();
      }).catch((err) => {
        error(err.message);
        reject(err);
