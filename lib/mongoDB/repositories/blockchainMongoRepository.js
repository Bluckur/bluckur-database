...
class BlockchainMongoRepository {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.BlockModel.find({}, { _id: 0, __v: 0 }, (err, blocks) => {
        if (err) {
          ...
          log(`${blocks.length} blocks fetched`);
          resolve(blocks);
        }
      });
    });
  ...
  putBlocksAsync(blocks) {
    return new Promise((resolve, reject) => {
      log(`putting blocks ... [blockNumbers=${blocks.map((b) => { return b.blockHeader.blockNumber; })}]`);
      this.BlockModel.insertMany(blocks, (err) => {
        if (err) {
          error(err.message);
          reject(err);
        } else {
          log(`${blocks.length} blocks put`);
          resolve();
        }
      });
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      this.BlockModel.deleteMany({ 'blockHeader.blockNumber': { $in: blockNumbers } }, (err) => {
        if (err) {
          ...
          log(`${blockNumbers.length} blocks deleted`);
          resolve();
        }
      });
    });
